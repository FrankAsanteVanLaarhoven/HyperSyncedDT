        ...
        with media_container:
            ...
            # YouTube Stream Tool
                ...
                with cols[0]:
                    ...
                    # Embedded YouTube stream preview
                    st.markdown("""
                    <div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; background: #f9f9f9;">
                        <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; align-items: center; justify-content: center;">
                            <div style="text-align: center;">
                                <svg width="100" height="70" viewBox="0 0 461.001 461.001">
                                ...
                                <p>YouTube Live Stream Preview</p>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    stream_tabs = st.tabs(["Stream Info", "Live Chat", "Analytics"])
                    
                    with stream_tabs[0]:
                        st.text_input("Stream Title", value="HyperSyncDT Manufacturing Demo")
                        st.text_area("Description", value="Live demonstration of our manufacturing digital twin platform.")
                        st.multiselect("Tags", ["manufacturing", "digital twin", "industry 4.0", "IoT", "automation"], 
                                      default=["manufacturing", "digital twin"])
                    ...
                    with stream_tabs[1]:
                        ...
                        **Live Chat**
                        ...
                        👤 **JohnDoe**: Is this system compatible with our CNC machines?  
                        👤 **MaryTech**: How does the predictive maintenance work?  
                        👤 **FactoryOps**: Great demonstration! Very impressive results.  
                        👤 **EngineerX**: What's the latency between the physical and digital twin?
                        """)
                        
                        chat_cols = st.columns([3, 1])
                        chat_message = chat_cols[0].text_input("Type a message")
                        chat_cols[1].button("Send", use_container_width=True)
                    
                    with stream_tabs[2]:
                        # Mock analytics
                        st.metric("Live Viewers", "24", "+8")
                        st.metric("Watch Time", "48 minutes", "+12%")
                        st.metric("Engagement", "86%", "+5%")
                
                with cols[1]:
                    st.markdown("### Stream Controls")
                    
                    # Stream status indicator
                    stream_status = st.selectbox("Stream Status", ["Ready", "Live", "Ended"])
                    if stream_status == "Ready":
                        st.button("Go Live", type="primary", use_container_width=True)
                    elif stream_status == "Live":
                        st.button("End Stream", type="primary", use_container_width=True)
                    else:
                        st.button("Start New Stream", type="primary", use_container_width=True)
                    
                    # Stream settings
                    st.subheader("Settings")
                    st.radio("Privacy", ["Public", "Unlisted", "Private"])
                    st.checkbox("Enable DVR")
                    st.checkbox("Auto-start when live")
                    st.slider("Stream Delay", 0, 30, 0, 5, format="%d sec")
